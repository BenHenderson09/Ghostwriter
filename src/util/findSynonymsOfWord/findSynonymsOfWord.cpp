...
namespace {
    ...
    std::vector<std::string> extractSynonymsFromResponse(const std::string& responseString);
    ...
    std::vector<std::string> extractSynonymsFromResponse(const std::string& responseString){
        ...
        bool doesResponseIncludeSynonyms{JSONObjectHasKey(jsonResponse, "synonyms")};    
